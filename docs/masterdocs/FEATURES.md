...
API: `GET /knowledge/search`, `GET /knowledge/docs/{id}`, `POST /knowledge/ingest` (admin), `POST /knowledge/qa` (legacy one-shot), `POST /knowledge/chat` plus `GET /knowledge/chats` and `GET /knowledge/chats/{id}/messages` (conversational assistant; see section 5 deltas).
...
- **Conversational knowledge assistant (replaces the one-shot Ask UI).** `POST /knowledge/chat` runs a conversation with per-conversation history and a rolling summary, retrieval over the cached corpus, and Anthropic API-native citations (`search_result` blocks): numbered markers are inserted server-side from the structured citation spans, each cited source lists its exact quoted passages, and retrieved-but-uncited sources are shown separately as related sources. Every answer ends with a "What the retrieved guidance does not cover" section, enforced by a contract validator with retry. The service lives in `services/qa_chat.py`, outside the LangGraph graphs; the one-shot `POST /knowledge/qa` remains.
- **Context harness.** Alongside the guidance, each turn rebuilds a live estate profile from the database (registers with estimate flags, task and timeline progress including comments, latest IHT assessment) and carries an estate-wide pool of pinned key passages across conversations, deduplicated and capped. The assistant reads and explains these; figures still come only from the registers and the engine, with estimates flagged as such.
- **Optional semantic search behind an admin parameter.** Embeddings are off by default (the local model, mixedbread mxbai-embed-large-v1 via fastembed, is a large download with a CPU load not every machine can run). An admin Params toggle enables them and starts a background backfill with progress and error reporting; retrieval adds the pgvector arm only while the toggle is on, and full-text RRF retrieval is the always-available baseline.
- **Corpus expansion.** The source registry grew to 77 entries: every document the app references elsewhere is now cached (NS&I bereavement and Premium Bonds guidance, Marie Curie, bereavement financial-support guides, missing-accounts services and more), so the assistant answers from the same sources the app points users at. The defunct Experian Unclaimed Assets Register (closed 2022) was replaced by Gretel app-wide.
- **AI guardrails as audited parameters.** A fast-model scope check pauses questions that look unrelated to estate administration for user confirmation (fail-open by design); a per-estate daily question limit returns HTTP 429 beyond the ceiling; SDK calls carry explicit timeouts and a single retry. Scope check and daily limit are admin-editable on the Params page, stored in `app_setting`, and every change is audited with before and after values. Bounds documented in SECURITY.md.
- **When you need help page.** A static, verified directory of 17 support organisations (emotional, practical and money, armed forces, tax and probate) with phone numbers as `tel:` links, hours and websites; numbers verified against each organisation's own site with the verification date shown, and honest "check the website" caveats where verification was blocked.
